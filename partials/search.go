...
func SearchSubmitButton() *h.Element {
	buttonClasses := "rounded dark:border-2 dark:border-white-200 items-center px-3 py-2 bg-white-800 text-white w-full text-center"
	return h.Div(
		h.HxBeforeRequest(
...
func SubmitForm(ctx *h.RequestContext) *h.Partial {
	...
	return h.NewPartial(
		h.Div(
			h.Class("text-base/8"),
			h.P(h.TextF("Searching for a book named: %s", name)),
			h.P(h.TextF("response: %s", string(body))),
		),
	)
}
